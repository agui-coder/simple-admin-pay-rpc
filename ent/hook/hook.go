...
// Mutate calls f(ctx, m).
func (f DemoOrderFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.DemoOrderMutation); ok {
		return f(ctx, mv)
	}
...
// The OrderExtensionFunc type is an adapter to allow the use of ordinary
// function as OrderExtension mutator.
type OrderExtensionFunc func(context.Context, *ent.OrderExtensionMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f OrderExtensionFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.OrderExtensionMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.OrderExtensionMutation", m)
}
